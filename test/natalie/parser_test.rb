...
describe 'Parser' do
  describe '#parse' do
    ...
    it 'parses operator expressions' do
      ...
      Parser.parse('x[:y] += 1').should == s(:block, s(:op_asgn1, s(:call, nil, :x), s(:arglist, s(:lit, :y)), :+, s(:lit, 1)))
    ...
    it 'parses assignment' do
      ...
      Parser.parse('*a = 1, 2').should == s(:block, s(:masgn, s(:array, s(:splat, s(:lasgn, :a))), s(:array, s(:lit, 1), s(:lit, 2))))
    end
    ...
    it 'parses while/until' do
      ...
      Parser.parse('x = 10; x -= 1 until x.zero?').should == s(:block, s(:lasgn, :x, s(:lit, 10)), s(:until, s(:call, s(:lvar, :x), :zero?), s(:lasgn, :x, s(:call, s(:lvar, :x), :-, s(:lit, 1))), true))
      Parser.parse('x = 10; x -= 1 while x.positive?').should == s(:block, s(:lasgn, :x, s(:lit, 10)), s(:while, s(:call, s(:lvar, :x), :positive?), s(:lasgn, :x, s(:call, s(:lvar, :x), :-, s(:lit, 1))), true))
    end
    ...
    it 'parses return' do
      ...
      Parser.parse('return 1, 2').should == s(:block, s(:return, s(:array, s(:lit, 1), s(:lit, 2))))
    ...
    it 'parses block' do
      ...
      Parser.parse("foo do |(x, (y, z))|\nend").should == s(:block, s(:iter, s(:call, nil, :foo), s(:args, s(:masgn, :x, s(:masgn, :y, :z)))))
      ...
      if RUBY_ENGINE == 'natalie'
        ...
        Parser.parse('bar { |a, | a }').should == s(:block, s(:iter, s(:call, nil, :bar), s(:args, :a), s(:lvar, :a)))
      else
        Parser.parse('bar { |a, | a }').should == s(:block, s(:iter, s(:call, nil, :bar), s(:args, :a, nil), s(:lvar, :a)))
      end
    end
    ...
    it 'parses break, next, super, and yield' do
      ...
      Parser.parse('next 1').should == s(:block, s(:next, s(:lit, 1)))
      ...
      Parser.parse('super 1').should == s(:block, s(:super, s(:lit, 1)))
      ...
      Parser.parse('yield()').should == s(:block, s(:yield))
      Parser.parse('yield 1').should == s(:block, s(:yield, s(:lit, 1)))
    ...
    it 'parses begin/rescue/else/ensure' do
      ...
      Parser.parse('def foo;0;rescue;:a;else;:c;ensure;:d;:e;end').should == s(:block, s(:defn, :foo, s(:args), s(:ensure, s(:rescue, s(:lit, 0), s(:resbody, s(:array), s(:lit, :a)), s(:lit, :c)), s(:block, s(:lit, :d), s(:lit, :e)))))
