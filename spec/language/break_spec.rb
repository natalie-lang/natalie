...
describe "The break statement in a lambda" do
  ...
  describe "when the invocation of the scope creating the lambda is still active" do
    ...
    # NATFIXME: break from proc-closure (LocalJumpError)
    xit "returns from the lambda" do
  ...
  describe "from a scope that has returned" do
    ...
    xit "raises a LocalJumpError when yielding to a lambda passed as a block argument" do
...
describe "The break statement in a module literal" do
  it "is invalid and raises a SyntaxError" do
    ...
    NATFIXME 'eval() only works on static strings', exception: SpecFailedException, message: 'eval() only works on static strings' do
...
describe "Executing break from within a block" do
  ...
  it "runs ensures when breaking from a loop" do
    ...
    NATFIXME 'it runs ensures when breaking from a loop', exception: SpecFailedException do
  ...
  # NATFIXME: break from proc-closure (LocalJumpError)
  xit "works when passing through a super call" do
