...
describe "Tempfile.create" do
  ...
  platform_is_not :windows do
    it "returns a private, readable and writable file" do
      @tempfile = Tempfile.create
      stat = @tempfile.stat
      NATFIXME 'Implement File::Stat#readable?', exception: NoMethodError, message: "undefined method 'readable?' for an instance of File::Stat" do
        stat.should.readable?
      end
      NATFIXME 'Implement File::Stat#writable?', exception: NoMethodError, message: "undefined method 'writable?' for an instance of File::Stat" do
        stat.should.writable?
      end
      NATFIXME 'Implement File::Stat#executable?', exception: NoMethodError, message: "undefined method 'executable?' for an instance of File::Stat" do
  ...
  context "when called with a single positional argument" do
    ...
    it "raises ArgumentError if passed something else than a String or an array of Strings" do
      ...
      -> { Tempfile.create([:create_spec]) }.should raise_error(ArgumentError, "unexpected prefix: :create_spec")
      NATFIXME 'Raise correct error class', exception: SpecFailedException, message: /should have raised ArgumentError,/ do
  ...
  context "when called with a second positional argument" do
    it "uses it as a directory for the tempfile" do
      @tempfile = Tempfile.create("create_spec", "./")
      NATFIXME 'Remove double slash', exception: SpecFailedException, message: /\A"\.\/\/create\S+ should start_with/ do
  ...
  context "when called with a mode option" do
    ...
    it "raises NoMethodError if passed a String mode" do
      NATFIXME 'it raises NoMethodError if passed a String mode', exception: SpecFailedException, message: /but instead raised nothing/ do
  ...
  ruby_version_is "3.4" do
    context "when called with anonymous: true" do
      it "returns an already unlinked File without a proper path" do
        NATFIXME 'Support anonymous keyword', exception: ArgumentError, message: 'unknown keyword: :anonymous' do
      ...
      it "unlinks file before calling the block" do
        NATFIXME 'Support anonymous keyword (error bubbles to ensure block)', exception: NoMethodError, message: "undefined method 'close!' for nil" do
  ...
  context "when called with other options" do
    it "passes them along to File.open" do
      ...
      NATFIXME 'it passes them along to File.open (possibly issue in File.open', exception: SpecFailedException do
