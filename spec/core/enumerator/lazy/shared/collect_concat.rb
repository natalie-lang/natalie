...
describe :enumerator_lazy_collect_concat, shared: true do
  ...
  describe "when the returned lazy enumerator is evaluated by Enumerable#first" do
    ...
    it "flattens elements when the given block returned an array or responding to .each and .force" do
      ...
      (0..Float::INFINITY).lazy.send(@method) { |n| (n * 10).to_s.each_char }.first(6).all? { |o| o.is_a? Enumerator }.should be_true
  ...
  describe "on a nested Lazy" do
    ...
    describe "when the returned lazy enumerator is evaluated by Enumerable#first" do
      ...
      it "flattens elements when the given block returned an array or responding to .each and .force" do
        ...
        (0..Float::INFINITY).lazy.map {|n| n * 10 }.send(@method) { |n| n.to_s.each_char }.first(6).all? { |o| o.is_a? Enumerator }.should be_true
