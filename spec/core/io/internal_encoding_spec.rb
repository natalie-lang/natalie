...
describe :io_internal_encoding, shared: true do
  ...
  describe "when Encoding.default_internal == Encoding.default_external" do
    ...
    it "returns nil regardless of Encoding.default_internal changes" do
      @io = new_io @name, @object
      Encoding.default_internal = Encoding::IBM437
      NATFIXME 'returns nil regardless of Encoding.default_internal changes', exception: SpecFailedException do
        @io.internal_encoding.should be_nil
      end
    end
  end
  ...
  describe "when Encoding.default_internal != Encoding.default_external" do
    ...
    it "returns nil when Encoding.default_external is BINARY and the internal encoding is not set" do
      ...
      @io = new_io @name, @object
      NATFIXME 'returns nil when Encoding.default_external is BINARY and the internal encoding is not set', exception: SpecFailedException do
        @io.internal_encoding.should be_nil
      end
    end
    ...
    it "returns nil when the external encoding is BINARY and the internal encoding is not set" do
      ...
      NATFIXME 'returns nil when the external encoding is BINARY and the internal encoding is not set', exception: SpecFailedException do
        @io.internal_encoding.should be_nil
      end
    end
  end
