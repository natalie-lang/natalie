...
describe "String#encode - to UTF-32BE" do
  context "from US-ASCII" do
    it "returns correct String in UTF-32BE" do
      a = "\x61".force_encoding("US-ASCII") # "a" - 0x61 (ASCII)
      a.encode("UTF-32BE").should == "\x00\x00\x00\x61".force_encoding("UTF-32BE")
    end
  end
  ...
  context "from ASCII-8BIT" do
    ...
    context "128-255" do
      it "raises Encoding::UndefinedConversionError" do
        ...
        -> { b.encode("UTF-32BE") }.should raise_error(
          Encoding::UndefinedConversionError,
          '"\x8F" to UTF-8 in conversion from ASCII-8BIT to UTF-8 to UTF-32BE'
        )
      end
    end
  end
  ...
  context "from UTF-8" do
    ...
    context "from 2-bytes UTF-8" do
      it "returns correct String in UTF-32BE" do
        ...
        c.encode("UTF-32BE").should == "\x00\x00\x00\xA5".force_encoding("UTF-32BE")
      end
    end
    ...
    context "from 3-bytes UTF-8" do
      it "returns correct String in UTF-32BE" do
        ...
        c.encode("UTF-32BE").should == "\x00\x00\x21\x1C".force_encoding("UTF-32BE")
      end
    end
    ...
    context "from 4-bytes UTF-8" do
      it "returns correct String in UTF-32BE" do
        ...
        c.encode("UTF-32BE").should == "\x00\x01\x01\x47".force_encoding("UTF-32BE")
      end
    end
  end

  context "from UTF-32BE" do
    it "returns a String in UTF-32BE with the same binary representation" do
      c = "\x00\x01\x01\x47".force_encoding("UTF-32BE") # U+10147
      c.encode("UTF-32BE").should == c
    end
  end
end
