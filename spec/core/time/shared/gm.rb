describe :time_gm, shared: true do
  ...
  it "creates a time based on given C-style gmtime arguments, interpreted as UTC (GMT)" do
    NATFIXME "Fix Time.#{@method}", condition: @method == :utc, exception: ArgumentError, message: 'wrong number of arguments (given 10, expected 1..7)' do
  ...
  guard -> {
    ...
    it "handles real leap seconds in zone 'right/UTC'" do
      with_timezone 'right/UTC' do
        NATFIXME "Implement Time.#{@method}", exception: ArgumentError, message: 'argument out of range' do
  ...
  it "handles bad leap seconds by carrying values forward" do
    with_timezone 'UTC' do
      NATFIXME "Fix Time.#{@method}", condition: @method == :utc, exception: ArgumentError, message: 'argument out of range' do
