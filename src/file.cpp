...
NatObject *File_initialize(Env *env, NatObject *self, ssize_t argc, NatObject **args, Block *block) {
    ...
    if (fileno == -1) {
        NatObject *exception_args[2] = { filename, integer(env, errno) };
        NatObject *error = send(env, const_get(env, NAT_OBJECT, "SystemCallError", true), "exception", 2, exception_args, NULL);
...
NatObject *File_expand_path(Env *env, NatObject *self, ssize_t argc, NatObject **args, Block *block) {
    ...
    NatObject *dotdot = regexp(env, "[^/]*/\\.\\./");
    NatObject *empty_string = string(env, "");
