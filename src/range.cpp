...
Value *Range_eqeq(Env *env, Value *self_value, ssize_t argc, Value **args, Block *block) {
    ...
    if (NAT_TYPE(args[0]) == Value::Type::Range) {
        ...
        if (truthy(send(env, self->range_begin, "==", 1, &arg->range_begin, NULL)) && truthy(send(env, self->range_end, "==", 1, &arg->range_end, NULL)) && self->range_exclude_end == arg->range_exclude_end) {
            return NAT_TRUE;
        }
    }
    return NAT_FALSE;
