...
NatObject *Range_eqeq(NatEnv *env, NatObject *self, size_t argc, NatObject **args, struct hashmap *kwargs, NatBlock *block) {
    ...
    if (NAT_TYPE(arg) == NAT_VALUE_RANGE && nat_truthy(nat_send(env, self->range_begin, "==", 1, &arg->range_begin, NULL)) && nat_truthy(nat_send(env, self->range_end, "==", 1, &arg->range_end, NULL)) && self->range_exclude_end == arg->range_exclude_end) {
        return NAT_TRUE;
    } else {
        return NAT_FALSE;
    }
