...
ValuePtr IoValue::read_file(Env *env, ValuePtr filename) {
    ...
    FileValue *file = _new(env, env->Object()->const_fetch(SymbolValue::intern("File"))->as_class(), 1, args, nullptr)->as_file();
...
ValuePtr IoValue::write(Env *env, size_t argc, ValuePtr *args) {
    ...
    for (size_t i = 0; i < argc; i++) {
        ...
        if (result == -1) {
            ...
            ExceptionValue *error = env->Object()->const_find(env, SymbolValue::intern("SystemCallError")).send(env, "exception", 1, &error_number, nullptr)->as_exception();
