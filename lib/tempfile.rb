...
class Tempfile
  class << self
    ...
    __define_method__ :create, [:basename], <<-END
      ...
      auto tmpdir = GlobalEnv::the()->Object()->const_fetch(SymbolValue::intern("Dir")).send(env, SymbolValue::intern("tmpdir"))->as_string();
      ...
      if (fileno == -1) {
          ...
          auto exception = GlobalEnv::the()->Object()->const_fetch(SymbolValue::intern("SystemCallError")).send(env, SymbolValue::intern("exception"), 1, args)->as_exception();
