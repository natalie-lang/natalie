...
class Fiddle
  ...
  class Handle
    __define_method__ :initialize, [:path], <<-END
      ...
      if (!handle) {
          auto dl_error = Object::const_find(env, self.klass(), "DLError"_s, Object::ConstLookupSearchMode::NotStrict).value().as_class();
  ...
  class Function
    ...
    __define_method__ :voidp_no_args, [], <<-END
      ...
      auto fn = (void* (*)())symbol;
      auto result = fn();
      auto pointer_class = Object::const_find(env, self.klass(), "Pointer"_s, Object::ConstLookupSearchMode::NotStrict).value().as_class();
