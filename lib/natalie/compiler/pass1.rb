  ...
  class Compiler
    ...
    class Pass1 < BasePass
      ...
      def transform_case(exp, used:)
        ...
        return transform_expression(else_block, used: used) if clauses.empty?

        instructions = []

        unless value
          return transform_case_when_without_value(exp, used: used)
        end

        instructions << transform_expression(value, used: true)
        clauses.each do |clause|
          case clause.sexp_type
          when :when
            instructions += transform_case_when(clause)
          when :in
            instructions += transform_case_in(clause)
          else
        ...
        if else_block
          instructions << transform_expression(else_block, used: true)
        else
          instructions << PushNilInstruction.new
        end

        instructions << [EndInstruction.new(:if)] * clauses.count { |c| c.sexp_type == :when }
      ...
      def transform_case_when_without_value(exp, used:)
