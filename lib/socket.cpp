...
Value Addrinfo_getaddrinfo(Env *env, Value self, Args &&args, Block *block) {
    ...
    for (struct addrinfo *rp = res; rp != nullptr; rp = rp->ai_next) {
        auto entry = self.send(env, "new"_s, { StringObject::create(reinterpret_cast<const char *>(rp->ai_addr), rp->ai_addrlen, Encoding::ASCII_8BIT), Value::integer(rp->ai_family), Value::integer(rp->ai_socktype) });
        if (rp->ai_canonname)
...
Value BasicSocket_getsockopt(Env *env, Value self, Args &&args, Block *block) {
    ...
    if (result == -1)
        env->raise_errno();
    auto data = StringObject::create(buf, len, Encoding::ASCII_8BIT);
...
Value BasicSocket_recv(Env *env, Value self, Args &&args, Block *) {
    ...
    return StringObject::create(buf, static_cast<size_t>(bytes));
}
...
Value TCPSocket_gethostbyname(Env *env, Value self, Args &&args, Block *) {
    ...
    for (auto ptr = hostent->h_addr_list; *ptr != nullptr; ptr++) {
        ...
        result->push(StringObject::create(ipaddr, Encoding::ASCII_8BIT));
    }
    return result;
}
