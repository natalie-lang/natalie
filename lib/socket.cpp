...
Value Addrinfo_getaddrinfo(Env *env, Value self, Args &&args, Block *block) {
    ...
    for (struct addrinfo *rp = res; rp != nullptr; rp = rp->ai_next) {
        auto entry = self.send(env, "new"_s, { StringObject::create(reinterpret_cast<const char *>(rp->ai_addr), rp->ai_addrlen, Encoding::ASCII_8BIT), Value::integer(rp->ai_family) });
        if (rp->ai_canonname)
