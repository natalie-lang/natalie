...
Value IPSocket_recvfrom(Env *env, Value self, Args args, Block *block) {
    args.ensure_argc_between(env, 1, 2);
    ...
    auto reverse_lookup = self.send(env, "do_not_reverse_lookup"_s).send(env, "!"_s);
    ...
    auto address = new ArrayObject { 4 };
    switch (src_addr.ss_family) {
    case AF_INET: {
        address->push(new StringObject("AF_INET"));
        const auto *in = reinterpret_cast<sockaddr_in *>(&src_addr);
        char host_buf[INET_ADDRSTRLEN];
        auto ntop_result = inet_ntop(in->sin_family, &in->sin_addr, host_buf, INET_ADDRSTRLEN);
        if (!ntop_result)
            env->raise_errno();
        address->push(Value::integer(ntohs(in->sin_port)));
        auto ip = new StringObject { host_buf };
        if (reverse_lookup->is_truthy()) {
            address->push(new StringObject(Socket_reverse_lookup_address(env, reinterpret_cast<sockaddr *>(&src_addr))));
        } else {
            address->push(ip);
        }
        address->push(ip);
    ...
    case AF_INET6: {
        address->push(new StringObject("AF_INET6"));
        const auto *in6 = reinterpret_cast<sockaddr_in6 *>(&src_addr);
        char host_buf[INET6_ADDRSTRLEN];
        auto ntop_result = inet_ntop(in6->sin6_family, &in6->sin6_addr, host_buf, INET6_ADDRSTRLEN);
        if (!ntop_result)
            env->raise_errno();
        address->push(Value::integer(ntohs(in6->sin6_port)));
        auto ip = new StringObject { host_buf };
        if (reverse_lookup->is_truthy()) {
            address->push(new StringObject(Socket_reverse_lookup_address(env, reinterpret_cast<sockaddr *>(&src_addr))));
        } else {
    ...
    default:
        NAT_NOT_YET_IMPLEMENTED("IPSocket#addr for family %d", src_addr.ss_family);
    }
    return new ArrayObject {
